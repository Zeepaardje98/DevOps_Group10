...
class courseServices():
    ...
    def mark_all_absent(courseData):
        ...
        if(not course):
            ...
            courses.update_one({**courseData},{"$push": {"attendance": {
    ...
    def checkStatus(attendance_array,roll_no):
        for attendance in attendance_array:
            if str(attendance['roll_no']) == str(roll_no):
        ...
        for attendance in attendances:
            ...
            if int(date.split('-')[1]) == int(month) + 1: ## CHECKING FOR THE MONTH
                ...
                ### If attendance of Just All Students is Requested
                    ...
                    roll_no_attended = {}
                    for el in current_day_attendance:
                        cur_roll = el["roll_no"]
                        cur_atten = el["status"]
                        roll_no_attended[cur_roll] = cur_atten
                    for roll_no in student_enrolled:
                        if not attendance_of_month.get(roll_no):
                            if roll_no_attended.get(roll_no, False):
                                attendance_of_month[roll_no] = {"totalClassesAttended": 1, "totalClasses":1}
                            else:
                            ...
                            attendance_of_month[roll_no] =  {
                                                                "totalClassesAttended": attendance_of_month[roll_no]['totalClassesAttended'] \
                                                                                            + int(courseServices.checkStatus(current_day_attendance,roll_no))
